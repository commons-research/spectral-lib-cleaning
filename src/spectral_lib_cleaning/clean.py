...
def run_clean(config: Config, input: str, output: str):
    ...
    workflow = create_workflow(
        yaml_file_name=os.path.join(
            str(config.processed_dir), "matchms_pipeline_settings.yaml"
        ),
        query_filters=[
    ...
    pipeline = Pipeline(
        ...
        logging_file=os.path.join(
            str(config.processed_dir), "library_cleaning_log.log"
        ),
